...
async def query_data(
    ...
    Args:
        ...
        metrics: List from ['impressions', 'clicks', 'spend', 'conversions', 'revenue', 'ctr', 'roas']
        group_by: Optional grouping: 'campaign', 'channel', 'date'
        source_ids: Filter to specific source UUIDs (all sources if omitted)
    ...
    selected_metrics = [metric_map[m].label(m) for m in metrics if m in metric_map]

    async with get_session(tenant_id) as session:
        if group_by == "campaign":
            stmt = (
                select(DimCampaign.name.label("campaign"), *selected_metrics)
            ...
            stmt = (
                select(DimChannel.name.label("channel"), *selected_metrics)
            ...
            stmt = (
                select(FactPerformance.date_key.label("date"), *selected_metrics)
                .where(
                    FactPerformance.tenant_id == tenant_id,
                    FactPerformance.date_key >= start,
                    FactPerformance.date_key <= end,
